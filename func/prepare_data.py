...
def get_data_from_data_gouv():
    ...
    if datetime.fromtimestamp(last_modified).date() != datetime.today().date():
        try:
            data_hospit = pd.read_csv(
                "https://www.data.gouv.fr/fr/datasets/r/63352e38-d353-4b54-bfd1-f1b3ee1cabd7",
                sep=";",
                parse_dates=["jour"],
                low_memory=False,
            )
            data_sidep = pd.read_csv(
        ...
        data_sidep = pd.read_csv(
            "data/data_sidep.csv", sep=";", parse_dates=["jour"], low_memory=False
        )
...
def prepare_data_hospit(study_day):
    ...
    merged_data = pd.merge(
        dataframe_day, sidep_day, left_index=True, right_index=True
    ).reset_index()
