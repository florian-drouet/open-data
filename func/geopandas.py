...
class geopandas_dataviz:
    ...
    def __init__(self, size, title):
        self.geojson_file = geopandas.read_file("data/departements.json").set_index(
            "code"
        )
        self.liste_dpt_idf = ["75", "77", "78", "91", "92", "93", "94", "95"]
        self.geojson_idf = self.geojson_file.loc[
            self.geojson_file.index.isin(self.liste_dpt_idf)
        ]
    ...
    def show_map(self, plot_column, shrink, idf, **kwargs):
        ...
        pd.merge(self.geojson_file, self.data, left_index=True, right_index=True).plot(
            column=plot_column,
            legend=True,
            ax=self.ax,
            legend_kwds={"shrink": shrink},
            **kwargs
        )
        if idf == True:
            self.geojson_idf.plot(
                ax=self.ax, facecolor="none", edgecolor="#888", linewidth=1
            )
        else:
            self.geojson_file.plot(
                ax=self.ax, facecolor="none", edgecolor="#888", linewidth=1
            )
